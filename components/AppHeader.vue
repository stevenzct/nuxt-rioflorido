    ...
    <nav :class="['fixed w-full z-20 top-0 start-0', { 'bg-white': isScrolled || isScrolledY }]" ref="nav">
      <div class="max-w-screen-2xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
          ...
          <button @click.prevent.stop="toggleNavbar" type="button"
            ...
            <svg v-if="!isNavbarOpen" class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 17 14">
        ...
        <div :class="['items-center justify-between', isNavbarOpen ? 'flex' : 'hidden', 'w-full md:flex md:w-auto md:order-1']" id="navbar-sticky">
          <ul id="navbar-links" class="flex flex-col md:p-0 mt-4 font-neue-montreal font-normal md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0">
            <li>
              <NuxtLink to="/" :class="[(isScrolled || isScrolledY) ? 'text-black' : 'text-white', 'relative inline-block group']" @click="toggleNavbar" aria-current="page">
                Home
                <span :class="[(isScrolled || isScrolledY) ? 'bg-black' : 'bg-white', 'absolute bottom-0 left-0 w-full h-[2px] transform scale-x-0 origin-bottom-right transition-transform duration-200 ease-out group-hover:scale-x-100 group-hover:origin-bottom-left']"></span>
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/projects" :class="[(isScrolled || isScrolledY) ? 'text-black' : 'text-white', 'relative inline-block group']" @click="toggleNavbar">
                Projects
                <span :class="[(isScrolled || isScrolledY) ? 'bg-black' : 'bg-white', 'absolute bottom-0 left-0 w-full h-[2px] transform scale-x-0 origin-bottom-right transition-transform duration-200 ease-out group-hover:scale-x-100 group-hover:origin-bottom-left']"></span>
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/services" :class="[(isScrolled || isScrolledY) ? 'text-black' : 'text-white', 'relative inline-block group']" @click="toggleNavbar">
                Services
                <span :class="[(isScrolled || isScrolledY) ? 'bg-black' : 'bg-white', 'absolute bottom-0 left-0 w-full h-[2px] transform scale-x-0 origin-bottom-right transition-transform duration-200 ease-out group-hover:scale-x-100 group-hover:origin-bottom-left']"></span>
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/about" :class="[(isScrolled || isScrolledY) ? 'text-black' : 'text-white', 'relative inline-block group']" @click="toggleNavbar">
                About
                <span :class="[(isScrolled || isScrolledY) ? 'bg-black' : 'bg-white', 'absolute bottom-0 left-0 w-full h-[2px] transform scale-x-0 origin-bottom-right transition-transform duration-200 ease-out group-hover:scale-x-100 group-hover:origin-bottom-left']"></span>
              </NuxtLink>
            </li>
            <li class="2xl:hidden xl:hidden lg:hidden md:hidden sm:block" id="contact-mobile" @click="toggleNavbar">
